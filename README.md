...
- `Processing/` — the internal tool that turns raw lecture/assignment photos
  into the PDFs above. See its own [README](Processing/README.md) for how to
  run it. Deliverables are moved out of its `final/` folder into `Homework/`
  or `Notes/` after each build; `final/` itself is regenerated on demand and
  not meant to hold the current copy.
